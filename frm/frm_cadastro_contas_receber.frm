...
Begin VB.Form frm_cadastro_contas_receber 
   ...
   Begin VB.Frame fme_filtros 
      ...
      Begin VB.ComboBox cbo_ordem 
         Height          =   315
         ItemData        =   "frm_cadastro_contas_receber.frx":0000
         Left            =   6780
         List            =   "frm_cadastro_contas_receber.frx":0002
      ...
      Begin VB.ComboBox cbo_ordenar_por 
         Height          =   315
         ItemData        =   "frm_cadastro_contas_receber.frx":0004
         Left            =   4380
         List            =   "frm_cadastro_contas_receber.frx":0006
         Style           =   2  'Dropdown List
         TabIndex        =   12
         Top             =   660
         Width           =   2295
   ...
   End
End
Attribute VB_Name = "frm_cadastro_contas_receber"
...
Private Enum enm_conta_receber
    col_vencimento = 0
...
Public Sub lsub_ajustar_grade(ByRef pgrd_grade As MSFlexGrid)
    ...
    With pgrd_grade
        ...
        .Cols = 7
        .Rows = 2
        .ColWidth(enm_conta_receber.col_vencimento) = 1110
        .ColWidth(enm_conta_receber.col_baixa_automatica) = 1450
        .ColWidth(enm_conta_receber.col_receita) = 2340
        .ColWidth(enm_conta_receber.col_valor) = 900
        .ColWidth(enm_conta_receber.col_parcela) = 900
        .ColWidth(enm_conta_receber.col_descricao) = 3495
        .ColWidth(enm_conta_receber.col_documento) = 1005
        .TextMatrix(0, enm_conta_receber.col_vencimento) = " Vencimento"
        .TextMatrix(0, enm_conta_receber.col_baixa_automatica) = " Baixa automática"
        .TextMatrix(0, enm_conta_receber.col_receita) = " Receita"
        .TextMatrix(0, enm_conta_receber.col_valor) = " Valor"
        .TextMatrix(0, enm_conta_receber.col_parcela) = " Parcela"
        .TextMatrix(0, enm_conta_receber.col_descricao) = " Descrição"
        .TextMatrix(0, enm_conta_receber.col_documento) = " Documento"
    End With
    stb_status.Panels(enm_status.pnl_mensagem).Text = "" 'limpa a barra de status
fim_lsub_ajustar_grade:
    Exit Sub
erro_lsub_ajustar_grade:
    psub_gerar_log_erro Err.Number, Err.Description, "frm_cadastro_contas_receber", "lsub_ajustar_grade"
...
erro_lsub_preencher_combos:
    psub_gerar_log_erro Err.Number, Err.Description, "frm_cadastro_contas_receber", "lsub_preencher_combos"
    GoTo fim_lsub_preencher_combos
End Sub

Private Sub lsub_cancelar_conta_receber(ByVal plng_codigo As Long)
    On Error GoTo erro_lsub_cancelar_conta_receber
    Dim lobj_cancelar_conta_receber As Object
    ...
    If (Not pfct_executar_comando_sql(lobj_cancelar_conta_receber, lstr_sql, "frm_cadastro_contas_receber", "lsub_cancelar_conta_receber")) Then
        MsgBox "Erro ao executar comando SQL.", vbOKOnly + vbCritical, pcst_nome_aplicacao
        GoTo fim_lsub_cancelar_conta_receber
    End If
    llng_registros = lobj_cancelar_conta_receber.Count
    If (llng_registros > 0) Then
        lstr_chave = lobj_cancelar_conta_receber(1)("str_chave")
        If (lstr_chave <> "") Then
            'monta o comando sql
            lstr_sql = "select count(*) as [int_quantidade] from [tb_contas_receber] where [str_chave] = '" & lstr_chave & "'"
            'executa o comando sql e devolve o objeto
            If (Not pfct_executar_comando_sql(lobj_ocorrencias, lstr_sql, "frm_cadastro_contas_receber", "lsub_cancelar_conta_receber")) Then
                MsgBox "Erro ao executar comando SQL.", vbOKOnly + vbCritical, pcst_nome_aplicacao
                GoTo fim_lsub_cancelar_conta_receber
            End If
            ...
            If (llng_registros > 0) Then
                ...
                If (llng_ocorrencias > 0) Then
                    ...
                    lstr_mensagem = lstr_mensagem & "' ocorrências desta conta a receber." & vbCrLf
                    ...
                    lstr_mensagem = lstr_mensagem & "- Não, excluir apenas a conta a receber selecionada." & vbCrLf
                    ...
                    If (lint_resposta = vbYes) Then
                        'monta o comando sql
                        lstr_sql = "delete from [tb_contas_receber] where [str_chave] = '" & lstr_chave & "'"
                        'executa o comando sql e devolve o objeto
                        If (Not pfct_executar_comando_sql(lobj_cancelar_conta_receber, lstr_sql, "frm_cadastro_contas_receber", "lsub_cancelar_conta_receber")) Then
                            MsgBox "Erro ao executar comando SQL.", vbOKOnly + vbCritical, pcst_nome_aplicacao
                            GoTo fim_lsub_cancelar_conta_receber
                    ...
                    ElseIf (lint_resposta = vbNo) Then
                        'monta o comando sql
                         lstr_sql = "delete from [tb_contas_receber] where [int_codigo] = " & pfct_tratar_numero_sql(plng_codigo)
                        'executa o comando sql e devolve o objeto
                        If (Not pfct_executar_comando_sql(lobj_cancelar_conta_receber, lstr_sql, "frm_cadastro_contas_receber", "lsub_cancelar_conta_receber")) Then
                            MsgBox "Erro ao executar comando SQL.", vbOKOnly + vbCritical, pcst_nome_aplicacao
                            GoTo fim_lsub_cancelar_conta_receber
        ...
        End If
    End If
fim_lsub_cancelar_conta_receber:
    lsub_preencher_combos
    lsub_ajustar_grade msf_grade
    stb_status.Panels(enm_status.pnl_mensagem).Text = "" 'limpa a barra de status
    'destrói os objetos
    Set lobj_cancelar_conta_receber = Nothing
    Set lobj_ocorrencias = Nothing
    Exit Sub
erro_lsub_cancelar_conta_receber:
    psub_gerar_log_erro Err.Number, Err.Description, "frm_cadastro_contas_receber", "lsub_cancelar_conta_receber"
    GoTo fim_lsub_cancelar_conta_receber
End Sub
...
Private Sub lsub_preencher_grade(ByVal pstr_data_de As String, _
    ...
    If (Not pfct_executar_comando_sql(lobj_contas_receber, lstr_sql, "frm_cadastro_contas_receber", "lsub_preencher_grade")) Then
        MsgBox "Erro ao executar comando SQL.", vbOKOnly + vbCritical, pcst_nome_aplicacao
    ...
    If (llng_registros > 0) Then
        ...
        For llng_contador = 1 To llng_registros
            ...
            msf_grade.Col = enm_conta_receber.col_vencimento
            msf_grade.RowData(llng_contador) = lobj_contas_receber(llng_contador)("int_codigo")
            msf_grade.TextMatrix(llng_contador, enm_conta_receber.col_vencimento) = " " & Format$(lobj_contas_receber(llng_contador)("dt_vencimento"), pcst_formato_data)
            msf_grade.TextMatrix(llng_contador, enm_conta_receber.col_baixa_automatica) = " " & IIf(lobj_contas_receber(llng_contador)("chr_baixa_automatica") = "S", "Sim", "Não")
            msf_grade.TextMatrix(llng_contador, enm_conta_receber.col_receita) = " " & lobj_contas_receber(llng_contador)("str_descricao_receita")
            msf_grade.TextMatrix(llng_contador, enm_conta_receber.col_valor) = " " & Format$(lobj_contas_receber(llng_contador)("num_valor"), pcst_formato_numerico)
            'ini parcelas
            msf_grade.TextMatrix(llng_contador, enm_conta_receber.col_parcela) = " " & _
                Format$(lobj_contas_receber(llng_contador)("int_parcela"), pcst_formato_numerico_parcela) & "/" & _
                Format$(lobj_contas_receber(llng_contador)("int_total_parcelas"), pcst_formato_numerico_parcela)
            'fim parcelas
            msf_grade.TextMatrix(llng_contador, enm_conta_receber.col_descricao) = " " & lobj_contas_receber(llng_contador)("str_descricao")
            msf_grade.TextMatrix(llng_contador, enm_conta_receber.col_documento) = " " & lobj_contas_receber(llng_contador)("str_documento")
            ...
            lcur_valor_total = lcur_valor_total + CCur(lobj_contas_receber(llng_contador)("num_valor"))
        Next
        msf_grade.Redraw = True
        msf_grade.Row = 1
        stb_status.Panels(enm_status.pnl_mensagem).Text = "Total de contas a receber de [" & Format$(dtp_de.Value, pcst_formato_data) & "] a [" & Format$(dtp_ate.Value, pcst_formato_data) & "] -> [" & Format$(llng_registros, "00") & "]" & " " & pfct_retorna_simbolo_moeda() & " " & Format$(lcur_valor_total, pcst_formato_numerico)
    Else
        MsgBox "Atenção!" & vbCrLf & "Não foram encontradas [contas a receber] no período selecionado.", vbOKOnly + vbInformation, pcst_nome_aplicacao
        stb_status.Panels(enm_status.pnl_mensagem).Text = "Não há contas a receber para o período selecionado."
        GoTo fim_lsub_preencher_grade
    End If
...
erro_lsub_preencher_grade:
    psub_gerar_log_erro Err.Number, Err.Description, "frm_cadastro_contas_receber", "lsub_preencher_grade"
    GoTo fim_lsub_preencher_grade
End Sub
...
erro_cbo_ordem_DropDown:
    psub_gerar_log_erro Err.Number, Err.Description, "frm_cadastro_contas_receber", "cbo_ordem_DropDown"
...
erro_cbo_ordem_GotFocus:
    psub_gerar_log_erro Err.Number, Err.Description, "frm_cadastro_contas_receber", "cbo_ordem_GotFocus"
...
erro_cbo_ordem_LostFocus:
    psub_gerar_log_erro Err.Number, Err.Description, "frm_cadastro_contas_receber", "cbo_ordem_LostFocus"
...
erro_cbo_ordenar_por_DropDown:
    psub_gerar_log_erro Err.Number, Err.Description, "frm_cadastro_contas_receber", "cbo_ordenar_por_DropDown"
...
erro_cbo_ordenar_por_GotFocus:
    psub_gerar_log_erro Err.Number, Err.Description, "frm_cadastro_contas_receber", "cbo_ordenar_por_GotFocus"
...
erro_cbo_ordenar_por_LostFocus:
    psub_gerar_log_erro Err.Number, Err.Description, "frm_cadastro_contas_receber", "cbo_ordenar_por_LostFocus"
...
Private Sub cmd_cancelar_Click()
    ...
    Else
        lsub_cancelar_conta_receber llng_codigo_item
    End If
fim_cmd_cancelar_Click:
    Exit Sub
erro_cmd_cancelar_Click:
    psub_gerar_log_erro Err.Number, Err.Description, "frm_cadastro_contas_receber", "cmd_cancelar_Click"
    GoTo fim_cmd_cancelar_Click
End Sub
...
Private Sub cmd_baixar_Click()
    ...
    Else
        frm_cadastro_contas_receber.Enabled = False
        frm_baixar_contas_receber.Left = frm_cadastro_contas_receber.Left + 250
        frm_baixar_contas_receber.Top = frm_cadastro_contas_receber.Top + 250
        frm_baixar_contas_receber.Show
    End If
fim_cmd_baixar_Click:
    Exit Sub
erro_cmd_baixar_Click:
    psub_gerar_log_erro Err.Number, Err.Description, "frm_cadastro_contas_receber", "cmd_baixar_Click"
    GoTo fim_cmd_baixar_Click
End Sub
...
erro_cmd_fechar_Click:
    psub_gerar_log_erro Err.Number, Err.Description, "frm_cadastro_contas_receber", "cmd_fechar_Click"
...
Private Sub cmd_filtrar_Click()
    ...
    Select Case cbo_ordenar_por.Text
        ...
        Case "- Lançamento"
            lstr_ordenar_por = "[tb_contas_receber].[int_codigo]"
        Case "- Vencimento"
            lstr_ordenar_por = "[dt_vencimento]"
        Case "- Receita"
            lstr_ordenar_por = "[tb_receitas].[int_codigo]"
        Case "- Descrição"
...
erro_cmd_filtrar_Click:
    psub_gerar_log_erro Err.Number, Err.Description, "frm_cadastro_contas_receber", "cmd_filtrar_Click"
    GoTo fim_cmd_filtrar_Click
End Sub
...
Private Sub cmd_lancar_Click()
    ...
    End If
    frm_cadastro_contas_receber.Enabled = False
    frm_lancar_contas_receber.Left = frm_cadastro_contas_receber.Left + 250
    frm_lancar_contas_receber.Top = frm_cadastro_contas_receber.Top + 250
    frm_lancar_contas_receber.Show
fim_cmd_lancar_Click:
    Exit Sub
erro_cmd_lancar_Click:
    psub_gerar_log_erro Err.Number, Err.Description, "frm_cadastro_contas_receber", "cmd_lancar_Click"
...
erro_dtp_ate_DropDown:
    psub_gerar_log_erro Err.Number, Err.Description, "frm_cadastro_contas_receber", "dtp_ate_DropDown"
...
erro_dtp_ate_GotFocus:
    psub_gerar_log_erro Err.Number, Err.Description, "frm_cadastro_contas_receber", "dtp_ate_GotFocus"
...
erro_dtp_ate_LostFocus:
    psub_gerar_log_erro Err.Number, Err.Description, "frm_cadastro_contas_receber", "dtp_ate_LostFocus"
...
erro_dtp_de_DropDown:
    psub_gerar_log_erro Err.Number, Err.Description, "frm_cadastro_contas_receber", "dtp_de_DropDown"
...
erro_dtp_de_GotFocus:
    psub_gerar_log_erro Err.Number, Err.Description, "frm_cadastro_contas_receber", "dtp_de_GotFocus"
...
erro_dtp_de_LostFocus:
    psub_gerar_log_erro Err.Number, Err.Description, "frm_cadastro_contas_receber", "dtp_de_LostFocus"
...
Erro_Form_Initialize:
    psub_gerar_log_erro Err.Number, Err.Description, "frm_cadastro_contas_receber", "Form_Initialize"
...
Fim_Form_KeyPress:
    Exit Sub
Erro_Form_KeyPress:
    psub_gerar_log_erro Err.Number, Err.Description, "frm_cadastro_contas_receber", "Form_KeyPress"
...
Private Sub Form_KeyUp(KeyCode As Integer, Shift As Integer)
    ...
    Select Case KeyCode
        Case vbKeyF1
            psub_exibir_ajuda Me, "html/financeiro_contas_receber.htm", 0
        Case vbKeyF2
            cmd_lancar_Click
        Case vbKeyF3
            cmd_cancelar_Click
...
Fim_Form_KeyUp:
    Exit Sub
Erro_Form_KeyUp:
    psub_gerar_log_erro Err.Number, Err.Description, "frm_cadastro_contas_receber", "Form_KeyUp"
    GoTo Fim_Form_KeyUp
End Sub
...
fim_Form_Load:
    Exit Sub
erro_Form_Load:
    psub_gerar_log_erro Err.Number, Err.Description, "frm_cadastro_contas_receber", "Form_Load"
    GoTo fim_Form_Load
End Sub
...
erro_msf_grade_MouseUp:
    psub_gerar_log_erro Err.Number, Err.Description, "frm_cadastro_contas_receber", "msf_grade_MouseUp"
...
erro_mnu_msf_grade_copiar_Click:
    psub_gerar_log_erro Err.Number, Err.Description, "frm_cadastro_contas_receber", "mnu_msf_grade_copiar_Click"
...
Private Sub mnu_msf_grade_exportar_Click()
    ...
    pfct_exportar_conteudo_grade msf_grade, "contas_a_receber"
fim_mnu_msf_grade_exportar_Click:
    Exit Sub
erro_mnu_msf_grade_exportar_Click:
    psub_gerar_log_erro Err.Number, Err.Description, "frm_cadastro_contas_receber", "mnu_msf_grade_exportar_Click"
