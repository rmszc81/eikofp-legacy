...
Begin VB.Form frm_baixar_contas_pagar 
   ...
   End
End
Attribute VB_Name = "frm_baixar_contas_pagar"
...
Private mlng_codigo_conta_pagar As Long
...
Private Function lfct_verificar_campos() As Boolean
    ...
    If (cbo_lancar_conta.ItemData(cbo_lancar_conta.ListIndex) = 0) Then
        ...
        GoTo Fim_lfct_verificar_campos
    End If
    'valida o combo tipo de despesa
...
Erro_lfct_verificar_campos:
    psub_gerar_log_erro Err.Number, Err.Description, "frm_baixar_contas_pagar", "lfct_verificar_campos"
    GoTo Fim_lfct_verificar_campos
End Function

Private Sub lsub_preencher_campos(ByVal plng_codigo As Long)
    On Error GoTo erro_lsub_preencher_campos
    Dim lobj_conta_pagar As Object
    Dim lstr_sql As String
    Dim llng_registros As Long
    Dim llng_contador As Long
    'atribui o código da conta a pagar na variável modular
    mlng_codigo_conta_pagar = plng_codigo
    'monta o comando sql
    ...
    If (Not pfct_executar_comando_sql(lobj_conta_pagar, lstr_sql, "frm_baixar_contas_pagar", "lsub_preencher_campos")) Then
        MsgBox "Erro ao executar comando SQL.", vbOKOnly + vbCritical, pcst_nome_aplicacao
        GoTo fim_lsub_preencher_campos
    End If
    llng_registros = lobj_conta_pagar.Count
    If (llng_registros > 0) Then
        'preenche os valores nas variáveis
        mlng_parcela = CLng(lobj_conta_pagar(1)("int_parcela"))
        mlng_total_parcelas = CLng(lobj_conta_pagar(1)("int_total_parcelas"))
        mbln_baixa_automatica = IIf(lobj_conta_pagar(1)("chr_baixa_automatica") = "S", True, False)
        mlng_conta_baixa_automatica = CLng(lobj_conta_pagar(1)("int_conta_baixa_automatica"))
        'preenche os valores nos campos
        txt_valor.Text = Format$(lobj_conta_pagar(1)("num_valor"), pcst_formato_numerico)
        txt_descricao.Text = lobj_conta_pagar(1)("str_descricao")
        dtp_vencimento.Value = CDate(lobj_conta_pagar(1)("dt_vencimento"))
        txt_documento.Text = lobj_conta_pagar(1)("str_documento")
        txt_codigo_barras.Text = lobj_conta_pagar(1)("str_codigo_barras")
        txt_observacoes.Text = lobj_conta_pagar(1)("str_observacoes")
        ...
        If (mlng_conta_baixa_automatica) Then
            ...
            For llng_contador = 0 To cbo_lancar_conta.ListCount - 1
                If (cbo_lancar_conta.ItemData(llng_contador) = lobj_conta_pagar(1)("int_conta_baixa_automatica")) Then
                    cbo_lancar_conta.ListIndex = llng_contador
                    Exit For
                End If
            Next
        End If
        'seleciona o item no combo de despesas
        For llng_contador = 0 To cbo_tipo_despesa.ListCount - 1
            If (cbo_tipo_despesa.ItemData(llng_contador) = lobj_conta_pagar(1)("int_despesa")) Then
                cbo_tipo_despesa.ListIndex = llng_contador
                Exit For
            End If
        Next
        'seleciona o item no combo formas de pagamento
        For llng_contador = 0 To cbo_forma_pagamento.ListCount - 1
            If (cbo_forma_pagamento.ItemData(llng_contador) = lobj_conta_pagar(1)("int_forma_pagamento")) Then
...
fim_lsub_preencher_campos:
    'destrói os objetos
    Set lobj_conta_pagar = Nothing
    Exit Sub
erro_lsub_preencher_campos:
    psub_gerar_log_erro Err.Number, Err.Description, "frm_baixar_contas_pagar", "lsub_preencher_campos"
    GoTo fim_lsub_preencher_campos
End Sub
...
erro_cbo_forma_pagamento_DropDown:
    psub_gerar_log_erro Err.Number, Err.Description, "frm_baixar_contas_pagar", "cbo_forma_pagamento_DropDown"
...
erro_cbo_forma_pagamento_GotFocus:
    psub_gerar_log_erro Err.Number, Err.Description, "frm_baixar_contas_pagar", "cbo_forma_pagamento_GotFocus"
...
erro_cbo_forma_pagamento_LostFocus:
    psub_gerar_log_erro Err.Number, Err.Description, "frm_baixar_contas_pagar", "cbo_forma_pagamento_LostFocus"
...
erro_cbo_lancar_conta_gotFocus:
    psub_gerar_log_erro Err.Number, Err.Description, "frm_baixar_contas_pagar", "cbo_lancar_conta_GotFocus"
...
erro_cbo_lancar_conta_LostFocus:
    psub_gerar_log_erro Err.Number, Err.Description, "frm_baixar_contas_pagar", "cbo_lancar_conta_LostFocus"
...
erro_cbo_lancar_conta_DropDown:
    psub_gerar_log_erro Err.Number, Err.Description, "frm_baixar_contas_pagar", "cbo_lancar_conta_DropDown"
...
erro_cbo_tipo_despesa_DropDown:
    psub_gerar_log_erro Err.Number, Err.Description, "frm_baixar_contas_pagar", "cbo_tipo_despesa_DropDown"
...
erro_cbo_tipo_despesa_gotFocus:
    psub_gerar_log_erro Err.Number, Err.Description, "frm_baixar_contas_pagar", "cbo_tipo_despesa_GotFocus"
...
erro_cbo_tipo_despesa_LostFocus:
    psub_gerar_log_erro Err.Number, Err.Description, "frm_baixar_contas_pagar", "cbo_tipo_despesa_LostFocus"
...
Private Sub cmd_baixar_Click()
    ...
    Dim lobj_remover_conta_pagar As Object
    ...
    If (lfct_verificar_campos) Then
        If (mbln_baixa_automatica) Then
            lint_resposta = MsgBox("Esta conta a pagar está programada para ser baixada automaticamente em sua data de vencimento." & vbCrLf & "Deseja baixar essa conta mesmo assim?", vbYesNo + vbQuestion + vbDefaultButton2, pcst_nome_aplicacao)
        Else
        ...
        If (lint_resposta = vbYes) Then
            ...
            If ((p_usuario.bln_lancamentos_duplicados) And (lstr_documento <> Empty)) Then 'só verificamos se houver também um número de documento
                ...
                'executa o comando sql e devolve o objeto
                If (Not pfct_executar_comando_sql(lobj_movimentacao, lstr_sql, "frm_baixar_contas_pagar", "cmd_baixar_Click")) Then
                    MsgBox "Erro ao executar comando SQL.", vbOKOnly + vbCritical, pcst_nome_aplicacao
            ...
            'executa o comando sql e devolve o objeto
            If (Not pfct_executar_comando_sql(lobj_conta, lstr_sql, "frm_baixar_contas_pagar", "cmd_baixar_Click")) Then
                MsgBox "Erro ao executar comando SQL.", vbOKOnly + vbCritical, pcst_nome_aplicacao
                GoTo fim_cmd_baixar_Click
            End If
            ...
            If (llng_registros > 0) Then
                ...
                If ((ldbl_saldo_atual - ldbl_valor_movimentacao) >= (ldbl_limite_negativo * -1)) Then
                    ...
                    'executa o comando sql e devolve o objeto
                    If (Not pfct_executar_comando_sql(lobj_atualizar_saldo_conta, lstr_sql, "frm_baixar_contas_pagar", "cmd_baixar_Click")) Then
                        MsgBox "Erro ao executar comando SQL.", vbOKOnly + vbCritical, pcst_nome_aplicacao
                        GoTo fim_cmd_baixar_Click
                    End If
                    ...
                    lstr_sql = lstr_sql & " ) "
                    'executa o comando sql e devolve o objeto
                    If (Not pfct_executar_comando_sql(lobj_baixar_conta_pagar, lstr_sql, "frm_baixar_contas_pagar", "cmd_baixar_Click")) Then
                        MsgBox "Erro ao executar comando SQL.", vbOKOnly + vbCritical, pcst_nome_aplicacao
                        GoTo fim_cmd_baixar_Click
                    End If
                    '-- exclui a conta a pagar da tabela --'
                    'monta o comando sql
                    lstr_sql = "delete from [tb_contas_pagar] where [int_codigo] = " & pfct_tratar_numero_sql(mlng_codigo_conta_pagar)
                    'executa o comando sql e devolve o objeto
                    If (Not pfct_executar_comando_sql(lobj_remover_conta_pagar, lstr_sql, "frm_baixar_contas_pagar", "cmd_baixar_Click")) Then
                        MsgBox "Erro ao executar comando SQL.", vbOKOnly + vbCritical, pcst_nome_aplicacao
                        GoTo fim_cmd_baixar_Click
                    End If
...
fim_cmd_baixar_Click:
    ...
    Set lobj_remover_conta_pagar = Nothing
    Set lobj_atualizar_saldo_conta = Nothing
    Exit Sub
erro_cmd_baixar_Click:
    psub_gerar_log_erro Err.Number, Err.Description, "frm_baixar_contas_pagar", "cmd_baixar_Click"
    GoTo fim_cmd_baixar_Click
End Sub
...
erro_cmd_cancelar_Click:
    psub_gerar_log_erro Err.Number, Err.Description, "frm_baixar_contas_pagar", "cmd_cancelar_Click"
...
erro_dtp_vencimento_GotFocus:
    psub_gerar_log_erro Err.Number, Err.Description, "frm_baixar_contas_pagar", "dtp_vencimento_GotFocus"
...
erro_dtp_vencimento_LostFocus:
    psub_gerar_log_erro Err.Number, Err.Description, "frm_baixar_contas_pagar", "dtp_vencimento_LostFocus"
...
erro_dtp_vencimento_DropDown:
    psub_gerar_log_erro Err.Number, Err.Description, "frm_baixar_contas_pagar", "dtp_vencimento_DropDown"
...
Fim_Form_KeyPress:
    Exit Sub
Erro_Form_KeyPress:
    psub_gerar_log_erro Err.Number, Err.Description, "frm_baixar_contas_pagar", "Form_KeyPress"
...
Private Sub Form_KeyUp(KeyCode As Integer, Shift As Integer)
    ...
    Select Case KeyCode
        Case vbKeyF1
            psub_exibir_ajuda Me, "html/financeiro_contas_pagar_baixar.htm", 0
        Case vbKeyF2
            cmd_baixar_Click
        Case vbKeyF3
            cmd_cancelar_Click
...
Fim_Form_KeyUp:
    Exit Sub
Erro_Form_KeyUp:
    psub_gerar_log_erro Err.Number, Err.Description, "frm_baixar_contas_pagar", "Form_KeyUp"
    GoTo Fim_Form_KeyUp
End Sub
...
Erro_Form_Initialize:
    psub_gerar_log_erro Err.Number, Err.Description, "frm_baixar_contas_pagar", "Form_Initialize"
...
Private Sub Form_Load()
    ...
    lsub_preencher_campos frm_cadastro_contas_pagar.msf_grade.RowData(frm_cadastro_contas_pagar.msf_grade.Row)
fim_Form_Load:
    Exit Sub
erro_Form_Load:
    psub_gerar_log_erro Err.Number, Err.Description, "frm_baixar_contas_pagar", "Form_Load"
    GoTo fim_Form_Load
End Sub
...
fim_Form_Unload:
    Exit Sub
erro_Form_Unload:
    psub_gerar_log_erro Err.Number, Err.Description, "frm_baixar_contas_pagar", "Form_Unload"
...
erro_txt_codigo_barras_GotFocus:
    psub_gerar_log_erro Err.Number, Err.Description, "frm_baixar_contas_pagar", "txt_codigo_barras_GotFocus"
...
erro_txt_codigo_barras_LostFocus:
    psub_gerar_log_erro Err.Number, Err.Description, "frm_baixar_contas_pagar", "txt_codigo_barras_LostFocus"
...
erro_txt_codigo_barras_validate:
    psub_gerar_log_erro Err.Number, Err.Description, "frm_baixar_contas_pagar", "txt_codigo_barras_validate"
...
erro_txt_descricao_validate:
    psub_gerar_log_erro Err.Number, Err.Description, "frm_baixar_contas_pagar", "txt_descricao_validate"
...
erro_txt_descricao_GotFocus:
    psub_gerar_log_erro Err.Number, Err.Description, "frm_baixar_contas_pagar", "txt_descricao_GotFocus"
...
erro_txt_descricao_LostFocus:
    psub_gerar_log_erro Err.Number, Err.Description, "frm_baixar_contas_pagar", "txt_descricao_LostFocus"
...
erro_txt_documento_GotFocus:
    psub_gerar_log_erro Err.Number, Err.Description, "frm_baixar_contas_pagar", "txt_documento_GotFocus"
...
erro_txt_documento_LostFocus:
    psub_gerar_log_erro Err.Number, Err.Description, "frm_baixar_contas_pagar", "txt_documento_LostFocus"
...
erro_txt_documento_validate:
    psub_gerar_log_erro Err.Number, Err.Description, "frm_baixar_contas_pagar", "txt_documento_validate"
...
erro_txt_observacoes_gotFocus:
    psub_gerar_log_erro Err.Number, Err.Description, "frm_baixar_contas_pagar", "txt_observacoes_GotFocus"
...
erro_txt_observacoes_LostFocus:
    psub_gerar_log_erro Err.Number, Err.Description, "frm_baixar_contas_pagar", "txt_observacoes_LostFocus"
...
erro_txt_observacoes_validate:
    psub_gerar_log_erro Err.Number, Err.Description, "frm_baixar_contas_pagar", "txt_observacoes_validate"
...
erro_txt_valor_GotFocus:
    psub_gerar_log_erro Err.Number, Err.Description, "frm_baixar_contas_pagar", "txt_valor_GotFocus"
...
erro_txt_valor_LostFocus:
    psub_gerar_log_erro Err.Number, Err.Description, "frm_baixar_contas_pagar", "txt_valor_LostFocus"
...
erro_txt_valor_validate:
    psub_gerar_log_erro Err.Number, Err.Description, "frm_baixar_contas_pagar", "txt_valor_validate"
